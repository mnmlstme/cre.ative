...
module.exports = {
  ...
  description: "Typescript (but not TSX) for using Lit",
  languages: {
...
const tsDefnRegex =
  /^\s*(function|class|let|const|interface|type)\s+(\w+)/;
...
function register({ providesLanguage, defaultModule }) {
  ...
  providesLanguage("js", {
    ...
    collate: (workbook) => {
      ...
      const buildDefn = ([n, _, code]) =>
        `// JS Definition from scene ${n + 1}\n${code}`;
      const buildScene = ([n, _, code]) =>
        `// JS scene ${n + 1}\n"${n}": function () { ${code} }`;
  ...
  providesLanguage("ts", {
    classify: (code) => classifyJavascript(code, tsDefnRegex),
    collate: (workbook) => {
      const { moduleName, imports } = workbook;
      const { scenes, definitions } = Kr.extract(
        workbook,
      ...
      return {
        ...
        name: "module.ts",
        language: "ts",
        code: `// module ${moduleName} (Typescript)
          ${imports.map(buildImport).join("\n")}
          console.log('Loading module "${moduleName}"')
          export function Program ({connectStore, initializeStore}) {
            ${definitions.map(buildDefn).join("\n")}
...
function classifyJavascript(code, regex = jsDefnRegex) {
  const keywordToType = {
    function: "function",
    const: "constant",
    var: "variable",
  ...
  const defnMatch = code.match(regex);
