...
export function loadWorkbook(projectId, workbookId) {
  ...
  return (dispatch, getState, { importModule }) => {
    ...
    const onHotSwap = (path) => {
      console.log('Hot swap notification for', path)
...
export function loadResource(defn) {
  const { use, filepath, language = 'js' } = defn

  return (dispatch, getState, { importResource }) => {
    console.log('loadResource', filepath)
    ...
    importResource(filepath, onHotSwap)
      .then((mod) => {
